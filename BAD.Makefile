...
#CM_FIND         := $(shell find -name '*.rsc' | sort)
#CHECKSUM        := $(shell md5sum $(CM_FIND))
...
#md5sum: $1
#    @SUM=$$(md5sum $$(find -name '*.rsc' | sort) $1);
#	echo $$1; > $@
#	 | cut -d' ' -f 1); \
#    echo $$SUM; > $2
#md5sum $(ALL_RSC)
...
%.rsc: @echo $@
#$(eval CHECKSUM := $(shell md5sum $(filter-out $(shell find -name '*.rsc' | sort 2>/dev/null)) $*),echo $(CHECKSUM) >$@)

#    	@SUM=$$(md5sum $$(find -name '*.rsc' | sort) $$(ALL_RSC));
#		echo $$SUM; > 2		
#		| \
#    	sed -e "s| \./||" -e 's|.rsc$||' | \
#    	jq --raw-input --null-input '[ inputs | split (" ") | { (.[1]): (.[0]) }] | add' $< >$@
